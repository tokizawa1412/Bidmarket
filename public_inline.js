...
/* ============================================================
   BidMarket Admin Dashboard COMPLETE UI
...
function adminTabs(active='overview'){
  ...
  return `<div class="adminTabs">
    ${['overview','users','auctions','transactions','logs','escrow'].map(t=>`<button class="${active===t?'gold':'light'}" onclick="loadAdmin('${t}')">${{overview:'ภาพรวม',users:'ผู้ใช้',auctions:'ประมูล',transactions:'ธุรกรรม',logs:'Log',escrow:'Escrow'}[t]}</button>`).join('')}
  </div>`;
}
...
async function loadAdmin(tab=adminTab||'overview'){
  if(!me||me.role!=='admin')return alert('เฉพาะ Admin');
  adminTab=tab;
  try{
...
async function loadAdminLogs(){
  ...
  adminBox.innerHTML=adminTabs('logs')+`<div class="adminToolbar"><input id="adminLogSearch" placeholder="ค้นหา Log" value="${escapeHtml(q)}"><button onclick="loadAdminLogs()">ค้นหา</button></div><table class="adminTable"><tr><th>เวลา</th><th>Admin/Actor</th><th>Action</th><th>Target</th><th>Details</th></tr>${j.audit_logs.map(l=>`<tr><td>${l.created_at?new Date(l.created_at).toLocaleString('th-TH'):'-'}</td><td>${escapeHtml(l.actor_name||String(l.actor_id||'-'))}</td><td>${escapeHtml(l.action||'-')}</td><td>${escapeHtml(l.target_type||'-')} #${escapeHtml(String(l.target_id||''))}</td><td><pre class="adminPre">${escapeHtml(JSON.stringify(l.details||{},null,2))}</pre></td></tr>`).join('')}</table>`;
}
async function loadAdminEscrow(){
  let e=await api('/api/admin/escrow');let pj=await api('/api/admin/payments');
  adminBox.innerHTML=adminTabs('escrow')+`<div class="grid"><div class="card content"><h3>เงินพัก Escrow</h3><div class="price">${money(e.held,'Credit')}</div></div><div class="card content"><h3>รอจัดส่ง</h3><div class="price">${e.waitShipping}</div></div><div class="card content"><h3>ข้อพิพาท</h3><div class="price">${e.disputes}</div></div></div>
