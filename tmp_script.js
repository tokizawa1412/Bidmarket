    ...
    <main class="accountMainV2">
      <section class="accountHeroBannerV2">
        <div class="accountIdentityCardV2">
          ...
          <div class="accountHeroName">${display} ${vipBadgeAssetHtml(vipLevelFromPointsLocked(me))}</div>
...
/* === VIP card/badge lock data and 1:1 account/profile layout override === */
const VIP_LEVEL_ORDER_LOCKED=['Member','Silver','Gold','Sapphire','Platinum','Diamond','Ruby','Elite'];
const VIP_LEVEL_THRESHOLDS_LOCKED={Member:0,Silver:100,Gold:2000,Sapphire:15000,Platinum:50000,Diamond:300000,Ruby:600000,Elite:1000000};
const VIP_CARD_ASSETS_LOCKED={member:'/assets/vip/vip-card-member.png',silver:'/assets/vip/vip-card-silver.png',gold:'/assets/vip/vip-card-gold.png',sapphire:'/assets/vip/vip-card-sapphire.png',platinum:'/assets/vip/vip-card-platinum.png',diamond:'/assets/vip/vip-card-diamond.png',ruby:'/assets/vip/vip-card-ruby.png',elite:'/assets/vip/vip-card-elite.png'};
const VIP_BADGE_ASSETS_LOCKED={member:'/assets/vip/vip-badge-member.png',silver:'/assets/vip/vip-badge-silver.png',gold:'/assets/vip/vip-badge-gold.png',sapphire:'/assets/vip/vip-badge-sapphire.png',platinum:'/assets/vip/vip-badge-platinum.png',diamond:'/assets/vip/vip-badge-diamond.png',ruby:'/assets/vip/vip-badge-ruby.png',elite:'/assets/vip/vip-badge-elite.png'};
function normalizeVipLevelLocked(level){const v=String(level||'Member').trim();return VIP_LEVEL_ORDER_LOCKED.find(x=>x.toLowerCase()===v.toLowerCase())||'Member'}
function vipRankLocked(level){return VIP_LEVEL_ORDER_LOCKED.indexOf(normalizeVipLevelLocked(level));}
function vipPointsLocked(u){return Number((u&&(u.vip_points??u.vipPoints??u.points))||0)}
function vipLevelFromPointsLocked(u){const p=vipPointsLocked(u);let lv=normalizeVipLevelLocked(u&&u.vip_level);for(const name of VIP_LEVEL_ORDER_LOCKED){if(p>=VIP_LEVEL_THRESHOLDS_LOCKED[name])lv=name}return lv}
function nextVipLevelLocked(level){const i=vipRankLocked(level);return VIP_LEVEL_ORDER_LOCKED[Math.min(i+1,VIP_LEVEL_ORDER_LOCKED.length-1)]}
function vipProgressLocked(u){const level=vipLevelFromPointsLocked(u);const next=nextVipLevelLocked(level);const p=vipPointsLocked(u);const target=VIP_LEVEL_THRESHOLDS_LOCKED[next]||VIP_LEVEL_THRESHOLDS_LOCKED[level]||0;let percent=target?Math.min(100,Math.max(0,(p/target)*100)):100;if(level==='Elite')percent=100;return {level,next,points:p,target,percent};}
function fmtVipNum(n){return Number(n||0).toLocaleString('th-TH')}
function vipBadgeAssetHtml(level){const l=normalizeVipLevelLocked(level).toLowerCase();return `<img class="vipBadgeImg" src="${VIP_BADGE_ASSETS_LOCKED[l]}" alt="${escapeHtml(normalizeVipLevelLocked(level))}">`}
function vipMiniBadge(u){const level=vipLevelFromPointsLocked(u||{});return vipBadgeAssetHtml(level)}
function vipPremiumCard(level){return vipBadgeAssetHtml(level||'Member')}
function accountVipCardHtml(u){
  const pr=vipProgressLocked(u||{});const key=pr.level.toLowerCase();const src=VIP_CARD_ASSETS_LOCKED[key]||VIP_CARD_ASSETS_LOCKED.member;
  const fill=Math.min(28.5,28.5*pr.percent/100);
  return `<div class="accountVipCard vipAssetCard vipLevel_${key}"><img class="vipCardBaseImg" src="${src}" alt="VIP ${escapeHtml(pr.level)}"><button class="vipHelpBtn" onclick="showVipLevelPopup()">?</button><div class="vipCardScoreOverlay"><div><div class="scoreLabel">คะแนนปัจจุบัน</div><div class="scoreNum">${fmtVipNum(pr.points)}</div></div><div><div class="scoreLabel">คะแนนเพื่อเลื่อนระดับ</div><div class="scoreNum">${fmtVipNum(pr.target)}</div></div></div><div class="vipCardProgressFill" style="width:${fill}%"></div><div class="vipCardPercentText">${pr.percent.toFixed(pr.percent%1?1:0)}%</div></div>`;
}
function showVipLevelPopup(){
  const pr=vipProgressLocked(me||{});const cur=pr.level.toLowerCase();const nxt=pr.next.toLowerCase();
  const condition=pr.level==='Elite'?`คุณอยู่ระดับสูงสุดแล้ว คะแนน VIP ปัจจุบัน ${fmtVipNum(pr.points)} คะแนน`:`ต้องมีคะแนน VIP สะสม ${fmtVipNum(pr.target)} คะแนน เพื่อเลื่อนจาก ${escapeHtml(pr.level)} ไปเป็น ${escapeHtml(pr.next)} (ตอนนี้มี ${fmtVipNum(pr.points)} คะแนน เหลืออีก ${fmtVipNum(Math.max(0,pr.target-pr.points))} คะแนน)`;
  helpTitle.innerHTML=`รายละเอียดการเลื่อนระดับ VIP <button class="vipPopupClose light" onclick="helpModal.classList.remove('show')">×</button>`;
  helpBody.className='vipLevelPopup';
  helpBody.innerHTML=`<div class="vipLevelCompare"><img src="${VIP_CARD_ASSETS_LOCKED[cur]}"><div class="vipArrow">→</div><img src="${VIP_CARD_ASSETS_LOCKED[nxt]}"></div><div class="vipCondition">${condition}</div>`;
  helpModal.classList.add('show');
}
